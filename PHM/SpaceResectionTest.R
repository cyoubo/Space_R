...
SpaceResectionTest_Data2<-function()
{
  ...
  landspoint=matrix(c(40589.0,26273.0,2195.0,
                      38589.0,26273.0, 728.0,
                      38589.0,28273.0, 757.0,
                      40589.0,28273.0,2386.0),4,3,byrow=T);
  
  Rmatrix=CoordinateSwitch_CreateRotateMatrix(0.002778,0,0,T);
...
Test_DistanceFromCenter_data1<-function()
{
  ...
  #利用已知外方位线元素（即摄影中心坐标）计算摄影中心到地面点的理论值
...
Test_Quaternion_RotateMatrix_data1<-function()
{
  ...
  #像平面精度为4位时，即通过共线方程计算所得数据
  photopoints=CoordinateSwitch_CollinearityEquation(data1[[5]],data1[[1]],data1[[2]],data1[[3]]);
  dis=SpaceResection_DistanceChange(photopoints,data1[[5]],data1[[1]]);
  PrintWithTitle("dis",dis);
